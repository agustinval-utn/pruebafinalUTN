...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_informe_1(self):
        ...
        for pokemon in lista:
            print(f"{i} - {pokemon}")
            i += 1
        "indice, recordar bien"
    
    def btn_mostrar_informe_2(self):
        contador_poder_mas_110 = 0
        for i, tipo in enumerate(self.lista_tipo_pokemones):
            if tipo == "fuego":
                print("es fuego")
                poder_mas_10 = self.lista_poder_pokemones[i] * 1.1
                poder_mas_10 = (self.lista_poder_pokemones[i] * 0.1) + self.lista_poder_pokemones[i]
                if poder_mas_10 > 100:
                    contador_poder_mas_110 += 1
                    print(f"{self.lista_nombre_pokemones[i]} - {self.lista_poder_pokemones[i]}")

    
    def btn_mostrar_informe_3(self):
        listapok = self.lista_nombre_pokemones
        i = 0
        for pokemon in listapok:
            print  (f"{i} - {pokemon}")
            i+= 1

        listatip = self.lista_tipo_pokemones
        i = 0
        for tipo in listatip:
            print(f"{i} - {tipo}")
            i += 1

        listapod = self.lista_poder_pokemones
        i = 0
        for poder in listapod:
            print (f"{i} - {poder}")
            i += 1
    ...
    def btn_cargar_pokedex_on_click(self):
        ...
        pokemon = self.lista_nombre_pokemones
        tipo = self.lista_tipo_pokemones
        poder_es_valido = False
        if poder:
            for numero in poder:
                if numero < 50:
                    poder_es_valido = False
